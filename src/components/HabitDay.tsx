...
import { generateProgresPercentage } from "../Utils/generate-progress-percentage";
...
export function HabitDay({ amountOfHabits = 0, amountCompleted = 0, date, ...rest }: Props) {
  const amountAccomplishedPercentage = amountOfHabits > 0 ? generateProgresPercentage(amountOfHabits, amountCompleted) : 0;
  const totay = dayjs().startOf('day').toDate();
  const isCurrentDay = dayjs(date).isSame(totay);
